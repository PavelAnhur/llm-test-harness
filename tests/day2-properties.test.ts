...
describe('Day 2: property-based assertion against an LLM', () => {
  ...
  it('always return an integer in [1, 100] when asked for a number', async () => {
    const prompt = 'Give me a random number between 1 and 100, with number only.';
    for (let i = 0; i < RUNS; i++) {
      const { text } = await generate(prompt, { temperature: TEMPERATURE });
      console.log(`Run ${i + 1}: ${text}`);
  ...
  it('always returns exactly two non-empty lines when asked for a poem', async () => {
    const prompt = 'Write a two-line poem about autumn. Without any intro. Just two lines.';
  ...
  it('returns valid JSON when asked for a structured output', async () => {
    const prompt = 'Return a valid JSON object with two fields: "color" (a color name) and "hex" (its hex code). Reply with JSON only, no prose.';
    ...
    for (let i = 0; i < RUNS; i++) {
      ...
      if (!json) {
        throw new Error(
          `No JSON object found in response, Raw text:\n${text}`
        );
      }
